...
export default function IndexChef() {
  ...
  const [projetMesTask,setProjetMesTask]= useState([])
  const [nbMesTask,setNbMesTask]= useState([])
  const [projetMesTaskRealise,setProjetMesTaskRealise]= useState([])
  const [nbMesTaskRealise,setNbMesTaskRealise]= useState([])
...
const getMesTaskUser = (async () => {
                await Axios.get(`http://localhost:8000/chefProjetsMesTasks/${user1.id}`
                )
                .then((response)=>{
                setNbMesTask(response.data.task)
                setProjetMesTask(response.data.projet)
        
                 })})
                useEffect( () => {getMesTaskUser();},[]);
const dataMesTaskUser = {
              labels: projetMesTask,
              datasets: [
                  ...
                  const getMesTaskRealiseUser = (async () => {
                    await Axios.get(`http://localhost:8000/chefProjetsMesTasksRealise/${user1.id}`
                    )
                    .then((response)=>{
                    setNbMesTaskRealise(response.data.task)
                    setProjetMesTaskRealise(response.data.projet)
            
                     })})
                    useEffect( () => {getMesTaskRealiseUser();},[]);
const dataMesTaskRealiseUser = {
                  labels: projetMesTaskRealise,
                  datasets: [
                      {label: "pourcentage des tâches réalisées %",
                      data: nbMesTaskRealise,
                      backgroundColor: ['#06868D'],
                      borderWidth: 1,},],    }  
return(
<>
                                ...
                                <div class="row mb-2 mt-3 " style={{fontSize:"14px"}}> 
                                  <div class="col-6" style={{fontWeight:'bold'}}> Mes projets ({nbTousProjetU}) et le nombre de mes tâches </div> 
                                  <div class="col-6" style={{fontWeight:'bold'}}> Mes projets ({nbTousProjetU}) et mes tâches réalisées(terminées et validées)</div> 
                                ...
                                <div class="row" >
                                  ...
                                  <div class="" style={{width:'550px', overflowX: 'auto'}}>
                                    ...
                                    <div class="" style={{height:'270px',width:'550px'}}>
                                     <Bar data={ dataMesTaskUser}  options={{scales: {y: {suggestedMin: 0,suggestedMax:10, ticks: {reverse: false,stepSize:1
                                                                },},},indexAxis: 'x',plugins:{title:{display:false, font:{size: 12, family: 'rubik'}},
                                                                 legend: {display: false, position: 'right'}}, maintainAspectRatio: false}} 
                                      /> 
                                    </div> 
                                  ...
                                  <div class="" style={{width:'550px', overflowX: 'auto'}}>
                                    ...
                                    <div class="" style={{height:'270px',width:'550px'}}>
                                     <Bar data={ dataMesTaskRealiseUser}  options={{scales: {y: {suggestedMin: 0,suggestedMax:100, ticks: {reverse: false,stepSize:10
                                                                },},},indexAxis: 'x',plugins:{title:{display:false, font:{size: 12, family: 'rubik'}},
                                                                 legend: {display: false, position: 'right'}}, maintainAspectRatio: false}} 
                                      /> 
                                    </div> 
