...
export default function EditTaskDuplicate() {
...
const onSubmitHandler = (e) => {
  e.preventDefault()
   const formData ={
        ...
        "blocage":blocageT,
        "user":userT,}
  Axios.post(`http://localhost:8000/dupliquerTask/${idT}`, formData,{
      headers: {
          "Content-Type": "multipart/form-data",
      }
  })
    .then((res) => {if(role==="ROLE_ADMIN"){window.location=`/tâcheProjet/${id}`} else{  window.location=`/tâcheProjet/${id}/${projetRole}`};
})
