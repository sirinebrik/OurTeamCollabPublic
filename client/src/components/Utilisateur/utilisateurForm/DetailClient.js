...
import ActivitésClient from "../../Activités/ActivitésClient"

export default function DetailClient({user,userRole}) {
  ...
  if (resetpassword){
    ...
    const handleClose = () => {setShow(false);setEmail(info.email);setNom(info.lastname) ;setError("");setPrenom(info.username);setSexe(info.sexe);setRole(info.roles[0]);
        setDepartement("");setSecteurCh("");setSecteurC(secteur.id);setNomE(client.nomEntreprise);setActivé(info.etat)}

    const getClient = (async () => {
          await Axios.get(`http://localhost:8000/detailClient/${id}`,).then((response)=>{
           setClient(response.data.client[0])
           setInfo(response.data.client[0].utilisateur)
           setRole(response.data.client[0].utilisateur.roles[0])
           setSecteur(response.data.client[0].secteur)
           setNom(response.data.client[0].utilisateur.lastname)
           setSecteurC(response.data.client[0].secteur.id)
           setNomE(response.data.client[0].nomEntreprise)
           setPrenom(response.data.client[0].utilisateur.username)
           setEmail(response.data.client[0].utilisateur.email)
           setSexe(response.data.client[0].utilisateur.sexe)
           setActivé(response.data.client[0].utilisateur.etat)
           setIsLoding(false);
...
const updatePassword = async (e) => {
  ...
  const changeOnClick = async (e) => {
    ...
    try {
      const res = await Axios.post(
        `http://localhost:8000/updateC/${id}`,
        formData,{
          headers: {
              "Content-Type": "multipart/form-data",
          }
      ...
      if(role==="ROLE_CHEFPROJET")window.location= `/profile/${id}`
      if(role==="ROLE_CLIENT")window.location= `./${id}`
   } catch (ex) {
            ...
            <div class="content-wrapper">
                <div class="row mb-4">
                    ...
                    {(userRole==="ROLE_ADMIN")&&(
                      ...
                      {(userRole==="ROLE_CLIENT")&&(user.id===info.id)&&(
                        <>
                             <div class="col-4">
                        ...
                        <div class="card card-rounded border border-3">
                            <div class="card-body">
                                <center style={{marginTop:"60px"}}>
                                   ...
                                   <div class="mt-3 mb-10" style={{fontSize:"16px",fontWeight:"bold"}}>{info.username}  {info.lastname}</div>
                            ...
                            <div class="card-body">
                             ...
                             {info.sexe==="homme" &&<p title="sexe" style={{color:"black"}}><i class=" me-1  fa fa-male" style={{fontSize:"13px",color:"#0B0A42"}}></i>Homme</p>}                                </div>
                              ...
                              <div class="row"> 
                                ...
                                <div class="col-6"> 
                                <p title="rôle" style={{color:"black"}}><i class=" me-1  fa fa-user" style={{fontSize:"13px",color:"#0B0A42"}}></i>Client</p>
                                  </div>
                              </div>
                              <div class="row"> 
                                <div class="col-6">
                                ...
                                <div class="col-6"> 
                                <p title="nom Entreprise" style={{color:"black" }}><i class=" me-1  fa fa-university " style={{fontSize:"13px",color:"#0B0A42"}}></i>{client.nomEntreprise}</p>
                                  </div>
                              </div>
                        ...
                        <div class="card card-rounded border border-3 " >
                            <div class="card-body">
                             ...
                             <div class="mb-1 mt-3" style={{fontSize:"13px",color:"#0B0A42",fontWeight:"bold"}}>Rôle</div>
                                ...
                                <p title="rôle" style={{color:"black"}}><i class=" me-1  fa fa-user" style={{fontSize:"13px",color:"#0B0A42"}}></i>Client</p>
                                <div class="mb-1 mt-3" style={{fontSize:"13px",color:"#0B0A42",fontWeight:"bold"}}>Entreprise</div>
                                <p title="nom Entreprise" style={{color:"black" }}><i class=" me-1  fa fa-university " style={{fontSize:"13px",color:"#0B0A42"}}></i>{client.nomEntreprise}</p>
                                <div class="" style={{fontSize:"13px",color:"#0B0A42",marginLeft:"6px"}}>Secteur d'activité :   <span title="secteur" style={{fontWeight:"bold"}}>{secteur.titre}</span></div>
                                <div class="mb-1 mt-3" style={{fontSize:"13px",color:"#0B0A42",fontWeight:"bold"}}>Compte </div>
                                   {info.etat===true && <span class="me-2" style={{fontSize:"13px",color:"#06868D",fontWeight:"bold"}}>Activé</span>} 
                                   {info.etat===false && <span class="me-2" style={{fontSize:"13px",color:"red",fontWeight:"bold"}}>Désactivé</span>}
                            </div>
                        </div>
                    </div>
                    <div class="col-8">
                       <div class="card card-rounded border border-3 " >
                            ...
                            <div class="row">
                               ...
                               <ActivitésClient id={id} type={type}/>
                            </div>
                        </div>
