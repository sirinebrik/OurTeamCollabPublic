...
import ActivitésMembre from "../../Activités/ActivitésMembre"

export default function DetailMembre({user,userRole}) {
  ...
  if (resetpassword){
    ...
    const handleClose = () => {setShow(false);setEmail(info.email);setNom(info.lastname) ;setError("");setPrenom(info.username);setSexe(info.sexe);setRole(info.roles[0]);
        setDepartement(membre.departement);setSecteur("");setSecteurC("");setNomE("");setActivé(info.etat)}
   
    const getMembre = (async () => {
       await Axios.get(`http://localhost:8000/detailMembre/${id}`,).then((response)=>{
           setMembre(response.data.membre[0])
           setInfo(response.data.membre[0].utilisateur)
           setNom(response.data.membre[0].utilisateur.lastname)
           setPrenom(response.data.membre[0].utilisateur.username)
           setEmail(response.data.membre[0].utilisateur.email)
           setSexe(response.data.membre[0].utilisateur.sexe)
           setDepartement(response.data.membre[0].departement)
           setActivé(response.data.membre[0].utilisateur.etat)
           setRole(response.data.membre[0].utilisateur.roles[0])
           setIsLoding(false);
...
const updatePassword = async (e) => {
  ...
  const changeOnClick = async (e) => {
    ...
    try {
      const res = await Axios.post(
        `http://localhost:8000/updateM/${id}`,
        formData,{
          headers: {
              "Content-Type": "multipart/form-data",
          }
      ...
      if(role==="ROLE_MEMBRE")window.location= `./${id}`
            ...
            <div class="content-wrapper">
                <div class="row mb-4">
                    ...
                    {(userRole==="ROLE_ADMIN")&&(
                      ...
                      {(userRole==="ROLE_MEMBRE")&&(user.id===info.id)&&(
                        <>
                           <div class="col-4">
                            ...
                            <div class="card-body">
                             ...
                             {info.sexe==="homme" &&<p title="sexe" style={{color:"black"}}><i class=" me-1  fa fa-male" style={{fontSize:"13px",color:"#0B0A42"}}></i>Homme</p>}                                </div>
                              ...
                              <div class="row"> 
                                ...
                                <div class="col-6"> 
                                <p title="rôle" style={{color:"black"}}><i class=" me-1  fa fa-user" style={{fontSize:"13px",color:"#0B0A42"}}></i>Membre</p>
                                  </div>
                              </div>
                              <div class="row"> 
                                <div class="col-6">
                                  <div class="mb-1 mt-1" style={{fontSize:"13px",color:"#0B0A42",fontWeight:"bold"}}>Département</div>
                               </div>
                                <div class="col-6"> 
                                  <p title="département" style={{color:"black" ,marginLeft:"6px"}}>{membre.departement}</p>
                                  </div>
                              </div>
                    ...
                    <div class="col-4"> 
                        <div class="card card-rounded border border-3">
                            <div class="card-body">
                              ...
                              <div class="mb-1 mt-3" style={{fontSize:"13px",color:"#0B0A42",fontWeight:"bold"}}>Infos</div>
                             {info.sexe==="femme" &&<p title="sexe" style={{color:"black"}}><i class=" me-1  fa fa-female" style={{fontSize:"13px",color:"#0B0A42"}}></i>Femme</p>}
                             {info.sexe==="homme" &&<p title="sexe" style={{color:"black"}}><i class=" me-1  fa fa-male" style={{fontSize:"13px",color:"#0B0A42"}}></i>Homme</p>}
                              <p title="rôle" style={{color:"black"}}><i class=" me-1  fa fa-user" style={{fontSize:"13px",color:"#0B0A42"}}></i>Membre</p>
                              <div class="mb-1 mt-3" style={{fontSize:"13px",color:"#0B0A42",fontWeight:"bold"}}>Département</div>
                              <p title="département" style={{color:"black" ,marginLeft:"6px"}}>{membre.departement}</p>
                    ...
                    <div class="col-8">
                       <div class="card card-rounded border border-3">
                            ...
                            <div class="row">
                               ...
                               <ActivitésMembre id={id} type={type}/>
                            </div>
                        </div>
                    </div> 
                </div>}
